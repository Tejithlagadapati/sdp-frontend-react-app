...
import { addService, deleteService, fetchAmenityServices, SERVICE_TYPES } from "../../services/serviceApi";
...
const Amenities = () => {
    ...
    const handleDelete = async (serviceId) => {
        const confirmed = window.confirm("Delete this amenity?");
        ...
        try {
            ...
            setError(deleteError?.response?.data?.message || deleteError.message || "Failed to delete amenity");
        } finally {
            setDeletingId(null);
        }
    };
            ...
            <div className="admin-amenity-grid">
                {!isLoading && filteredAmenities.map((item) => (
                    <Card key={item.id} className="admin-amenity-card">
                        ...
                        <button
                            type="button"
                            className="table-action-btn danger-action-btn"
