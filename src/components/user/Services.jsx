...
const Services = () => {
  ...
  const loadCategory = async (loader) => {
    try {
      return { data: await loader(), error: "" };
    } catch (loadError) {
      return {
        data: [],
        error: loadError?.response?.data?.message || loadError.message || "Failed to load service data",
      };
    }
  };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [servicesResult, infraResult, amenitiesResult] = await Promise.all([
          loadCategory(fetchPublicServices),
          loadCategory(fetchInfrastructureServices),
          loadCategory(fetchAmenityServices),
        ]);
        ...
        const errors = [servicesResult.error, infraResult.error, amenitiesResult.error].filter(Boolean);
        if (errors.length > 0) {
